...
data = pd.read_csv("https://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/data_csv.csv")
print(data)
print(stats.chisquare(data['관측도수']))
# statistic = 20.488188, pvalue=0.00039991
exp = [data['관측도수'].sum() / 5 ] * 5
print(exp)      # 기대빈도 : 50.8
stat, p, dof, expected = stats.chisquare(data['관측도수'], f_exp=exp)
print('stat : ', stat)
print('p : ', p)

# 판정 : 유의수준 0.05 < pvalue0.00039991 이므로 귀무 기각.
...
x = np.arange(len(data))    # 음료 수만큼 x 축 좌표
width = 0.35    # 막대 너비

plt.figure(figsize=(10, 5))
plt.bar(x - width / 2, data['관측도수'], width=width, label='관측도수')
plt.bar(x + width / 2, expected, width=width, label='기대도수', alpha=0)
